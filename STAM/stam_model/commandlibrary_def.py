#!/usr/bin/env python
"""
    Container class for storing the contents of an CommandLibrary statement in the STAM language.

    This file contains the contents of what was parsed from a STAM file for what was discovered in the
    CommandLibrary statement.  The objects of this class will be contained in the module objects as individual
    instances of each of the command library interfaces defined.  This class also defines visitor methods for each
...
class CommandLibraryDef:
    ...
    def visit_xml(self, fp: TextIOBase):
        ...
        ind.incr()
        s = ' '*ind.get_indent()
        s += '<commandlibrary name="' + self.name + '" >\n'
        fp.write(s)
        for i in self.items:
            i.visit_xml(fp)
        s = ' '*ind.get_indent()
