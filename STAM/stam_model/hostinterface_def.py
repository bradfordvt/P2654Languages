#!/usr/bin/env python
"""
    Container class for storing the contents of an HostInterface statement in the STAM language.

    This file contains the contents of what was parsed from a STAM file for what was discovered in the
    HostInterface statement.  The objects of this class will be contained in the module objects as individual
    instances of each of the host interfaces defined.  This class also defines visitor methods for each type of
...
class HostInterfaceDef:
    def __init__(self, name: HostInterfaceName = None, items: list = []):
        if name is not None:
            self.name = name
        else:
            self.name = None
    ...
    def visit_xml(self, fp: TextIOBase):
        ...
        s = ' '*ind.get_indent()
        if self.name:
            s += '<hostinterface name="' + self.name.get_name() + '" >\n'
        else:
            s += '<hostinterface name="*undefined*" >\n'
        fp.write(s)
        for i in self.items:
            i.visit_xml(fp)
        s = ' '*ind.get_indent()
