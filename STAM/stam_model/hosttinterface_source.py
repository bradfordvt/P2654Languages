#!/usr/bin/env python
"""
    Container class for storing the contents of an HostInterface_source statement in the STAM language.

    This file contains the contents of what was parsed from a STAM file for what was discovered in the
    HostInterface_source statement.  The objects of this class will be contained in the module objects as individual
    instances of each of the host interface source defined.  This class also defines visitor methods for each type of
    ...
    def visit_xml(self, fp: TextIOBase) -> None:
        ...
        if isinstance(self.name, HostInterfaceName):
            if self.name.get_index() is not None:
                s += ' <hostinterfacesource name="' + self.name.get_name() + '[' + str(self.name.get_index()) + ']"'
            else:
                s += ' <hostinterfacesource name="' + self.name.get_name() + '"'
        else:
            s += ' <hostinterfacesource name="' + str(self.name) + '"'
