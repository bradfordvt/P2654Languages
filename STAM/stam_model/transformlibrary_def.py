#!/usr/bin/env python
"""
    Container class for storing the contents of an TransformLibrary statement in the STAM language.

    This file contains the contents of what was parsed from a STAM file for what was discovered in the
    TransformLibrary statement.  The objects of this class will be contained in the module objects as individual
    instances of each of the transform library interfaces defined.  This class also defines visitor methods for each
...
class TransformLibraryDef:
    ...
    def visit_xml(self, fp: TextIOBase):
        ...
        ind.incr()
        s = ' '*ind.get_indent()
        s += '<transformlibrary name="' + self.name + '" >\n'
        fp.write(s)
        for i in self.items:
            i.visit_xml(fp)
        s = ' '*ind.get_indent()
